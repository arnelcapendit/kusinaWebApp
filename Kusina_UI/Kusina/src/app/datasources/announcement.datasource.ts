...
export class AnnouncementsDataSource extends DataSource<AnnouncementsReport> {
    ...
    loadAnnouncementReport(search, sortDirection, sortActive, pageIndex, pageSize, dateRange, reportType, filter, currentParam) {
        ...
        this.announcementService.getAnnouncementsData(search, sortDirection, pageIndex, pageSize, dateRange, reportType).pipe(
            catchError(() => of([])),
            finalize(() => this.loadingSubject.next(false)),
        )
        .subscribe(data => {
            //console.log(data);
            this.totalData = this.announcementService.totalData;
